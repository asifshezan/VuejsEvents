    ...
    <div class="w-full p-10 bg-gradient-to-br from-pink-50 to-indigo-100 grid place-items-center">
      <div class="w-3/4 mx-auto rounded border">
        <div class="bg-white p-10 shadow-sm">
          ...
          <p class="text-sm font-black text-gray-600 my-3">
          ...
          <div class="transition hover:bg-indigo-50" v-for="(accordion, index) in accordions" :key="index">
            <!-- header -->
            <div class="accordion-header text-black cursor-pointer transition flex space-x-5 px-5 items-center h-16" @click="accordion.isOpen =! accordion.isOpen">
              <i class="fas text-black" :class="accordion.isOpen ? 'fa-minus' : 'fa-plus'"></i>
              <h3>{{ accordion.heading }}</h3>
            </div>
            <!-- Content -->
            <div class="px-5 pt-0 text-left text-black pb-5" :class="accordion.isOpen ? '' : 'hidden'">
